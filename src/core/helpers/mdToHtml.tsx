  ...
  function renderMarkdown() {
    return markdown.split('\n').map((doc: string) => {
      ...
      if (finishAnalysesCode) {
        ...
        fullCode = '';
        return (
          <pre className="p-2 my-3 bg-gray-700 text-white rounded-md ">
            <code>{local}</code>
          </pre>
        );
      }
      ...
      if (doc.startsWith('# ')) {
        return <h1 className="text-5xl font-bold text-gray-700 my-3">{doc.slice(2, doc.length)}</h1>;
      }

      if (doc.startsWith('## ')) {
        return <h2 className="text-4xl font-bold text-gray-700 my-3">{doc.slice(3, doc.length)}</h2>;
      }

      if (doc.startsWith('### ')) {
        return <h3 className="text-3xl font-bold text-gray-700 my-3">{doc.slice(4, doc.length)}</h3>;
      }

      if (doc.startsWith('#### ')) {
        return <h4 className="text-2xl font-bold text-gray-700 my-3">{doc.slice(5, doc.length)}</h4>;
      }
      ...
      return <p className=" text-lg text-gray-600 my-2">{allData}</p>;
    });
  }
